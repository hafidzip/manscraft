...
// (kept in sync with src/game/fps/HeldBlockTool.ts — see there for docs)
...
export class HeldBlockTool {
  ...
  private buildTorchModel(): THREE.Group {
    ...
    g.traverse((obj: any) => {
      ...
      if (isFlame) obj.material = new THREE.MeshBasicMaterial({ color: c.clone() });
    });
  ...
  update(dt: number, visible: boolean, speedN: number) {
    ...
    this.group.position.set(0.34 + swayX - p * 0.05, -0.3 + swayY - p * 0.07, -0.52 - p * 0.06);
    const rx = 0.32 + p * 0.5, ry = -0.62 + p * 0.25, rz = 0.12;
